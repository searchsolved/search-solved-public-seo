...
df_live = pd.read_excel("/python_scripts/migration_mapper/pre.xlsx",
                        usecols=["Address", "Status Code", "Title 1", "H1-1"], dtype=str)
df_staging = pd.read_excel("/python_scripts/migration_mapper/post.xlsx",
                           usecols=["Address", "Status Code", "Title 1", "H1-1", "Indexability"], dtype=str)
...
df_3xx_export = df_live[
    (
        df_live["Status Code"].isin(
            [
                "300",
                "301",
                "302",
...
# drops any rows which are not 2xx or 4xx - source 3xx will be exported to its own file for review
df_live = df_live[
    (
        df_live["Status Code"].isin(
            [
...
df_3xx_export.to_csv('/python_scripts/migration_mapper/auto-migration-non-redirectable-urls.csv', index=False)
